...
class JyotishTests(unittest.TestCase):
    ...
    def test_all_accessibility_themes_are_shipped(self):
        css = Path("styles.css").read_text()
        javascript = Path("example.js").read_text()
        for theme in ("dawn", "midnight", "lotus", "contrast"):
            self.assertIn(f'data-theme="{theme}"', css)
            self.assertIn(f"'{theme}'", javascript)

...
const THEMES = ['dawn', 'midnight', 'lotus', 'contrast'];

function applyTheme(theme, persist = true) {
  const selected = THEMES.includes(theme) ? theme : 'dawn';
  document.documentElement.dataset.theme = selected;
  document.documentElement.style.colorScheme = selected === 'midnight' || selected === 'contrast' ? 'dark' : 'light';
  ...
  if (mobileTheme) mobileTheme.value = selected;
  if (persist) localStorage.setItem('jyotish-theme', selected);
}

const initialTheme = localStorage.getItem('jyotish-theme')
  || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'midnight' : 'dawn');
...
$('#mobileTheme').addEventListener('change', (event) => applyTheme(event.target.value));
